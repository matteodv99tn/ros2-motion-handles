...
//
void handles::add_axis_control(InteractiveMarker &marker,
...
void handles::prepare_motion_marker(InteractiveMarker &marker,
                                    const double &sphere_size) {
...
#define DECLARE_NODE_PARAMS(param)                                             \
  declare_parameter<decltype(params::value::param)>(params::name::param,       \
                                                    params::value::param);
...
PoseHandle::PoseHandle() : rclcpp::Node("pose_motion_handle") {
  ...
  const std::string marker_name =
      get_parameter(params::name::mrk_name).as_string();
  std::string marker_namespace =
      get_parameter(params::name::mrk_ns).as_string();

  if (marker_namespace.empty())
    marker_namespace = marker_name;

  RCLCPP_INFO(
      get_logger(),
      "Creating marker '%s' with reference frame '%s' on namespace '%s'",
      marker_name.c_str(), rf_name.c_str(), marker_namespace.c_str());

  // Set-up pose publisher
  if (!output_topic_name.empty()) {
    RCLCPP_INFO(get_logger(), "Pose broadcasted on topic %s",
                output_topic_name.c_str());
    _pose_publisher =
        create_publisher<PoseStamped>(output_topic_name, rclcpp::QoS(10));
  } else
    _pose_publisher = nullptr;
  ...
  _marker_server->setCallback(
      _marker_msg.name, [this](auto fbk) { this->_on_motion_update(fbk); },
...
void PoseHandle::_on_motion_update(
    const InteractiveMarkerFeedback::ConstSharedPtr &fbk) {
